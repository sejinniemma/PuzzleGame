'use strict';

const gameTimer = document.querySelector('.game__timer');
const gamePuzzle = document.querySelector('.game__puzzle');
const gameStartBtn = document.querySelector('.game__start--button');
const gameText = document.querySelector('.game__text');

let puzzleCount = 16;

 // make puzzle in Javascript
 Array(puzzleCount).fill().forEach((_,index) => {
     const puzzlePiece = document.createElement('li');
     puzzlePiece.setAttribute('data-index', index);
     puzzlePiece.classList.add(`list${index}`);

    gamePuzzle.appendChild(puzzlePiece);
});

